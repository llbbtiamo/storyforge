...
export function ProjectWorldSettingsListingSection({
  items,
  loading,
  onEdit,
  onDelete,
    ...
    <>
      <SectionCard>
        <DataTable
          ...
          emptyDescription="当前项目还没有世界设定。"
          emptyTitle="暂无世界设定"
          renderDetails={(item) => <JsonPreview title="content" value={item.content} />}
          rowKey="id"
        />
        {loading && items.length ? <div className="pt-4 text-sm text-text-muted">正在刷新世界设定列表…</div> : null}
      </SectionCard>
